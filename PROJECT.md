...
- **Last updated:** 2026-07-01 (bus mTLS — slice 19)
...
- [ ] Bus hardening: network-isolate Redpanda so the plaintext listeners are unreachable except via the proxy (as `pgnet` did for Postgres); per-client bus SVIDs; production SPIRE node attestation (join-token is dev-only).
