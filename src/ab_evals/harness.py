...
class EvalSet:
    ...
    min_score: float  # required fraction of passing cases (in addition to no critical fails)
...
def evaluate(model: Model, eval_set: EvalSet) -> EvalReport:
    """Run every case in the set against the model. Deterministic; no side effects."""
    results = []
