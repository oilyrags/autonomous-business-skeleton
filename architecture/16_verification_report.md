...
| 4 | **Compliance (lawful basis)** | **PASS** (build-proven) | `08` inventories personal-data elements with lawful basis; `09` enforces purpose-limitation in OPA; `04` flags personal events. | build-time control DONE (`ab_compliance`, ADR-0024, `make compliance` in CI): the gate fails if a `personal` event/field lacks an `08` RoPA record, a lawful basis, or a resolvable retention policy; cross-checks the RoPA, the code data-inventory, and the event catalog. |
...
| 6 | **Security** | **PASS** (build-proven) | `10`: agent identities as principals, least-privilege OPA, Vault secrets, immutable audit log, kill switch (3 scopes), prompt-injection + exfiltration controls. | build-time controls DONE: kill-switch drill within SLA (slice 04); prompt-injection — a sensitive tool fails closed on an untrusted-input flow (ADR-0021); exfiltration — an egress tool is blocked from transmitting data above its clearance (ADR-0022). Injection/exfiltration suites are representative + extensible. |
| 7 | **Finance** | **PASS** (build-proven) | `03`/`06` (AM-09..13): append-only ledger, deterministic math, payment caps + allow-list, maker-checker + SoD, idempotency. | build-time controls DONE (`ab_ledger`, ADR-0023, `make ledger` in CI): ledger-balance invariant (`trial_balance()==0`, integer minor units, no floats); double-payment failure-injection (replayed idempotency key posts nothing, DB-enforced); maker-checker + SoD (over-cap payment needs a checker ≠ maker). Deferred: payment allow-lists, multi-currency. |
...
| 9 | **AI** | **PASS** (build-proven) | `11`: gateway-only access, task profiles + deterministic fallbacks, eval-gated promotion, grounding + abstention, bias monitoring for significant decisions, traces, model cards. | build-time controls DONE (`ab_evals`, ADR-0018/0020, `make eval` in CI): (1) eval gate blocks a prompt-injection leaker + a low-capability model; gateway serves only promoted models. (2) per-profile **grounding threshold** enforced — a hallucinating model is blocked. (3) **bias eval wired for Art.22** — the significant-decision profile carries a fairness eval, the gate *requires* one for Art.22 profiles, and a biased model is blocked. |
...
| 12 | **Failure-injection** | **PASS** (build-proven) | Scenarios defined below; controls exist in design. | build-time DONE (`ab_failsim`, ADR-0025/0028, `make failsim` in CI): the suite injects each scenario against the real controls — **6 of 7 CONTAINED** with 0 breaches (remediated AM-11 new-payee approval; DSAR erasure-with-legal-hold now build-proven via `ab_compliance.dsar`); only the incident-rollback scenario is DEFERRED (SRE component not yet built), not passed. |
...
**Accepted as a buildable Mode-B design.** Originally 5 `CONDITIONAL` audits whose closure is mechanical; all five originally-`CONDITIONAL` audits are now **build-proven** — **9 (AI)** (eval gate + grounding + Art.22 bias), **6 (security)** (kill-switch + prompt-injection + exfiltration), **7 (finance)** (balance invariant + double-payment + maker-checker), **4 (compliance)** (lawful-basis RoPA gate), and **12 (failure-injection)** (`ab_failsim` scenario suite, 5/7 contained + 2 phase-deferred) — leaving **0 `CONDITIONAL`**. No `REMEDIATION` (open design gap) remains. Each is a `make` target in CI. The remaining design-only item is the deferred incident-rollback failure scenario (SRE component lands in a later phase); DSAR erasure-with-legal-hold is now build-proven (`ab_compliance.dsar`, ADR-0028). The conditions are exactly the build-time controls that the design *requires to be proven*, not weaknesses in the design — and several (kill switch, ledger balance, DSAR completeness, payment caps) are explicit launch blockers.
