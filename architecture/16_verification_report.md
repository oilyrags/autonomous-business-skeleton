...
**Accepted as a buildable Mode-B design.** Originally 5 `CONDITIONAL` audits whose closure is mechanical; **Audit 9 (AI)** (eval gate + grounding + Art.22 bias), **Audit 6 (security)** (kill-switch + prompt-injection + exfiltration), **Audit 7 (finance)** (balance invariant + double-payment + maker-checker), and **Audit 4 (compliance)** (lawful-basis RoPA gate) are now **build-proven**, leaving **1 `CONDITIONAL`** (12 failure-injection). No `REMEDIATION` (open design gap) remains. The conditions are exactly the build-time controls that the design *requires to be proven*, not weaknesses in the design — and several (kill switch, ledger balance, DSAR completeness, payment caps) are explicit launch blockers.
