...
| 12 | **Failure-injection** | **PASS** (build-proven) | Scenarios defined below; controls exist in design. | build-time DONE (`ab_failsim`, ADR-0025/0028/0029, `make failsim` in CI): the suite injects each scenario against the real controls — **all 7 CONTAINED, 0 breach, 0 deferred** (remediated AM-11 new-payee approval; DSAR erasure-with-legal-hold via `ab_compliance.dsar`; incident/rollback via `ab_ops.reliability`). |
