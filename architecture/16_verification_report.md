...
| 9 | **AI** | CONDITIONAL | `11`: gateway-only access, task profiles + deterministic fallbacks, eval-gated promotion, grounding + abstention, bias monitoring for significant decisions, traces, model cards. | build-time: **eval gate blocks a known-bad model — DONE** (`ab_evals`, ADR-0018: gate blocks a prompt-injection leaker + a low-capability model; gateway serves only promoted models; `make eval` in CI). Remaining: grounding metric threshold enforced; bias eval wired for Art.22 profiles. |
