...
| 12 | **Failure-injection** | **PASS** (build-proven) | Scenarios defined below; controls exist in design. | build-time DONE (`ab_failsim`, ADR-0025, `make failsim` in CI): the suite injects each scenario against the real controls — 5 of 7 CONTAINED with 0 breaches (remediated AM-11 new-payee approval); DSAR-erasure and incident-rollback scenarios reported DEFERRED (components not yet built), not passed. |
