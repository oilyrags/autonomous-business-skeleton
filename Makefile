...
ideate-multiagent: ## multi-agent GLM-5.2 ideation demo — 3 generators → critic → synthesizer → gate (canned, no infra)
	PYTHONPATH=src uv run python -m ab_growth.multiagent_demo

ideate-async: ## async streaming ideation demo — start → per-agent SSE frames → gated cards (canned, no infra)
	PYTHONPATH=src uv run python -m ab_growth.ideation_runner_demo
